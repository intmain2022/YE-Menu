...
namespace big
{
	...
	bool hooks::increment_stat_event(CNetworkIncrementStatEvent* net_event, CNetGamePlayer* sender, int64_t a3)
	{
		...
		if (features::antiIncreEvent)
		{
			if (Stats == rage::joaat("MPPLY_GAME_EXPLOITS")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~游戏漏洞利用~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_VC_HATE")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~语音聊天(仇恨言论)~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_VC_ANNOYINGME")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~语音聊天(骚扰)~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_EXPLOITS")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~漏洞利用~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_TC_ANNOYINGME")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~文字聊天(骚扰我)~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_TC_HATE")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~文字聊天(仇恨言论)~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_GRIEFING")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~恶意破坏~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_OFFENSIVE_TAGPLATE")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~攻击性玩家~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_OFFENSIVE_LANGUAGE")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~冒犯语言~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_OFFENSIVE_UGC")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~具有攻击性的UGC~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_BAD_CREW_NAME")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~不好的玩家名称~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_BAD_CREW_MOTTO")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~不好的玩家签名~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_BAD_CREW_STATUS")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~不好的玩家状态~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else if (Stats == rage::joaat("MPPLY_BAD_CREW_STATUS")) {
				features::notifyfmt(u8"~r~YE举报事件保护~s~~n~事件名称:~r~不好的玩家标志~s~~n~发送者:~r~%s~", PLAYER::GET_PLAYER_NAME(senderId));
				return true;
			}
			else
			{
	...
	bool HK_RID_CRASH_FUNC(__int64 a1) {
		return false;
	}
			...
			if (isggj) {
				if (player->is_valid())
				{
					g_fiber_pool->queue_job([=]
					{
						...
						features::notifyfmt(u8"~r~YE检测到广告机~s~~n~玩家:~b~%s~s~~n~措施:~r~崩溃", player->get_name());
					});
				}
				LOG(INFO) << "YE检测到广告机玩家:" << player->get_name() << "措施:崩溃";
			}
		}
	...
	hooking::hooking() :
		...
		m_rid_crash_hook("rid_crash_hook", g_pointers->m_Rid_Crash, &HK_RID_CRASH_FUNC),
